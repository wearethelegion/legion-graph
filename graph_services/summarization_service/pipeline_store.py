"""Postgres storage for Summarization Service (Service 3).
...
class SummarizationStore:
    """Postgres store for summarization pipeline data.
    ...
    async def ensure_tables(self) -> None:
        """Create processing_checkpoints table if it doesn't exist.
        ...
        logger.info("SummarizationStore: tables ensured")
    ...
    async def save_checkpoint(
        ...
        Critical: This must be called AFTER summary is generated and published,
    ...
    async def check_checkpoint(
        ...
        - If no, returns True (skip)
        - If yes, writes checkpoint BEFORE processing (BUG!) and returns False

        Returns:
            True if content matches existing checkpoint (skip processing)
            False if new/changed (needs processing)
        """
        existing_hash = await self._pool.fetchval(
            """
            SELECT content_hash
        ...
        await self._pool.execute(
            ...
            stage,
            content_hash,
            "",  # ingestion_id not used in dedup logic, kept for tracking
        )
