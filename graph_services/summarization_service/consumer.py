"""Kafka consumer for Summarization Service (Service 3).

Consumes EnrichedChunkMessage from enriched-code-chunks topic,
processes via streaming worker pool, publishes TextSummaryEvent
to text-summaries topic, and emits summarization_complete to
...
from .models import TextSummaryEvent, PipelineEvent
from .processor import SummarizationProcessor
...
class SummarizationConsumer:
    ...
    3. Each worker: dequeue → summarize → upsert → publish → loop
    ...
    def __init__(
        ...
        # Track ingestions that already emitted summarization_complete
    ...
    async def consume(self) -> None:
        ...
        Workers handle summarization + publishing.
        """
            ...
            while self._running:
                ...
                for partition_msgs in messages.values():
                    for msg in partition_msgs:
                        ...
                        if not all([event.company_id, event.chunk_id, event.content]):
                            logger.warning(
                        ...
                        if event.content_type == "code" and not event.project_id:
                            logger.warning(
                                "consumer.missing_required_fields",
                                chunk_id=event.chunk_id,
                                has_content=bool(event.content),
    ...
    async def _worker_loop(self, worker_id: int) -> None:
        """Persistent worker: dequeue → summarize → publish → loop.
    ...
    async def _process_chunk(self, chunk: EnrichedChunkMessage) -> None:
        """Process single chunk: set context → summarize → upsert → publish → check completion."""
        ...
        # Summarize (calls processor._summarize_with_retries)
        event = await self._processor._summarize_with_retries(chunk, 0, 1)

        if event is None:
            logger.error(
                "worker.summarization_failed",
                chunk_id=chunk.chunk_id,
    ...
    async def _check_and_emit_completion(
        ...
        try:
            ...
            if is_complete and total is not None:
                ...
                # Get summary counts for the completion event
                ...
                completion_event = PipelineEvent(
                    ...
                    total_summaries=counters.get("summaries_produced", 0),
                )
                ...
                logger.info(
                    "consumer.summarization_complete",
                    ingestion_id=ingestion_id,
                    chunks_processed=received,
                    total_chunks=total,
                    total_summaries=counters.get("summaries_produced", 0),
