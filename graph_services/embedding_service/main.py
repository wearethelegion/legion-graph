"""Entrypoint for Embedding Service (Service 4).
...
async def main() -> None:
    ...
    logger.info(
        "main.starting",
        input_topics=config.KAFKA_INPUT_TOPICS,
        output_topic=config.KAFKA_OUTPUT_TOPIC,
        batch_size=config.EMBEDDING_BATCH_SIZE,
        max_workers=config.MAX_PARALLEL_WORKERS,
    )

    # Configure Cognee (embedding settings, env vars)
    ...
    store = EmbeddingStore(pool)
    await store.ensure_tables()
    logger.info("main.tables_ensured")

    # Initialize processor (embedding engine is lazy-loaded on first use)
