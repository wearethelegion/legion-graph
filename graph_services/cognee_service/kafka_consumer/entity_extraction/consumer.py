"""Kafka consumer for entity extraction from enriched code chunks.

Consumes EnrichedChunkMessage from enriched-code-chunks topic and processes
batches through entity extraction only (no summarization).
Consumer group: entity-extraction-processor (independent from enriched-chunks-processor).
"""
...
class EntityExtractionKafkaConsumer:
    """Kafka consumer that reads EnrichedChunkMessage and extracts entities."""

    def __init__(
        self,
        processor: EntityExtractionProcessor,
        stats_tracker: CogniStatsTracker | None = None,
        config: type[EntityExtractionConsumerConfig] = EntityExtractionConsumerConfig,
    ):
    ...
    async def consume(self) -> None:
        ...
        Collects messages into batches of BATCH_SIZE grouped by
                            ...
                            if self.stats_tracker and event.ingestion_id:
                                try:
                                    await self.stats_tracker.record_skipped_file(
                                        ...
                                        file_path=event.file_path,
                                        service="entity_extraction_consumer",
