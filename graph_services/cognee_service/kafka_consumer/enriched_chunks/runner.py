"""Runner for Enriched Chunks Kafka Consumer.

Entrypoint: python -m cognee_service.kafka_consumer.enriched_chunks.runner
"""
...
from .consumer import EnrichedChunksKafkaConsumer
...
async def main() -> None:
    """Start the enriched chunks Kafka consumer."""
    config = EnrichedChunksConsumerConfig
    config.validate()
    ...
    # This replaces the old partial User.metadata.create_all approach which only
    # covered models imported at that point. create_db_and_tables() covers everything.
    ...
    try:
        from .metadata_writer import CogneeMetadataWriter
    ...
    # Initialize consumer
    consumer = EnrichedChunksKafkaConsumer(
        processor=processor,
